...
app.use(cors({
    origin: ['http://localhost:3000', 'https://startup-l5l8r39eh-tuan-anhs-projects-770b952f.vercel.app'],
...
// get views of post
app.get('/api/startups/views/:id', async (req: Request, res: Response) => {
    const id = req.params.id;
    try {
        const viewsOfPost = await StartupModel.findByIdAndUpdate(id,
            { $inc: { views: 1 } },
            { new: true }
        )
            .select(['views'])
...
app.get('/api/user/checkHasAuthor/:id', async (req: Request, res: Response) => {
...
// get details of user
app.get('/api/user/getProfile/:id', async (req: Request, res: Response) => {
...
app.post('/api/user/createAuthor', async (req: Request, res: Response) => {
...
app.post('/api/startup/createPost', async (req: Request, res: Response) => {
